...
export default function MyFollowingPost() {
  ...
  useEffect(()=> {
    ...
    const fetchData = async () => {
      ...
      try{const res = await fetch("https://insta-clone-backend-1.onrender.com/myfollowingpost",{
        method:"GET",
        headers: {
  ...
  const makeComment = async (id,commented) => { 
    try{const response = await fetch("https://insta-clone-backend-1.onrender.com/comment",{
      method: 'PUT',
      headers: {
        "Content-Type": "application/json",
        Authorization: "Bearer " + localStorage.getItem("token") 
