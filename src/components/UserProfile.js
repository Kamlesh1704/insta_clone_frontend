...
export default function UserProfile() {
  ...
  useEffect(()=> {
    const fetchData = async () => {
      try{
        const response = await fetch(`https://insta-clone-backend-1.onrender.com/user/${userid}`,{
        headers: {
  ...
  const followUser = async userId => {
    const response = await fetch("https://insta-clone-backend-1.onrender.com/follow",{
      method: 'PUT',
      headers: {
  ...
  const unfollowUser = async userId => {
    const response = await fetch("https://insta-clone-backend-1.onrender.com/unfollow",{
      method: 'PUT',
      headers: {
        'Content-Type':'application/json',
        Authorization: 'Bearer ' + localStorage.getItem("token")
